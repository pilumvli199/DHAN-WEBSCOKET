...
class MultiStockDhanBot:
    def __init__(self):
        ...
        logger.info(f"Bot initialized with {len(STOCK_MAPPING)} stocks")
    
    def get_live_data_single(self, sec_id, segment, name):
        try:
            response = self.dhan.get_ltp_data(exchange_segment=segment, security_id=str(sec_id))
            if isinstance(response, dict) and 'data' in response:
                ...
                if ltp > 0:
                    ...
                    return {
                        'name': name, 'ltp': float(ltp), 'open': float(data.get('open', 0)),
                        'high': float(data.get('high', 0)), 'low': float(data.get('low', 0)),
                        'close': float(prev_close), 'volume': data.get('volume', 0),
    ...
    def get_live_data_batch(self):
        try:
            ...
            logger.info(f"Fetching {len(WATCHLIST['NSE_EQ'])} stocks...")
            count = 0
            ...
            logger.info(f"Fetched {count} stocks and {len(parsed_data['IDX_I'])} indices")
    ...
    def get_intraday_data(self, sec_id, segment="NSE_EQ", name="Stock"):
        try:
            ...
            response = self.dhan.intraday_minute_data(
                security_id=str(sec_id), exchange_segment=segment, instrument_type=instrument_type
            )
            ...
            logger.error(f"Intraday error for {name}: {e}")
            return None
    ...
    def generate_candlestick_chart(self, candle_data, title="Stock Chart"):
        try:
            ...
            for i, candle in enumerate(data):
                ...
                height = abs(c - o)
                bottom = min(o, c)
                rect = Rectangle((i - 0.3, bottom), 0.6, height, facecolor=color, edgecolor=color)
                ax1.add_patch(rect)
            ...
            colors = ['#26a69a' if data[i]['close'] >= data[i]['open'] else '#ef5350' for i in range(len(data))]
            ax2.bar(range(len(data)), [d['volume'] for d in data], color=colors, alpha=0.5, width=0.8)
        ...
        try:
            if underlying_id not in self.current_expiry:
                try:
                    expiry_response = self.dhan.get_expiry_list(
                        exchange_segment=segment, security_id=str(underlying_id)
                    )
            ...
            if isinstance(response, dict) and 'data' in response:
                ...
                for strike in selected_strikes:
                    ...
                    option_data.append({
                        'strike': strike, 'ce_ltp': ce_data.get('ltp', 0),
                        'ce_oi': ce_data.get('oi', 0), 'ce_volume': ce_data.get('volume', 0),
                        'pe_ltp': pe_data.get('ltp', 0), 'pe_oi': pe_data.get('oi', 0),
                        'pe_volume': pe_data.get('volume', 0), 'is_atm': (strike == atm_strike)
                    })
                
                return {
                    'spot': spot_price, 'atm': atm_strike,
                    'expiry': self.current_expiry[underlying_id], 'options': option_data
                }
            return None
        except Exception as e:
    ...
    async def send_live_summary(self, live_data):
        try:
            ...
            logger.error(f"Send summary error: {e}")
    ...
    async def send_option_chain_message(self, name, option_data):
        try:
            ...
            message += "```"
            await self.bot.send_message(chat_id=TELEGRAM_CHAT_ID, text=message, parse_mode='Markdown')
            logger.info(f"OC sent for {name}")
        except Exception as e:
            logger.error(f"Send OC error: {e}")
    ...
    async def send_chart(self, name, sec_id, segment):
        try:
            ...
            if key in self.last_chart_time:
                if now - self.last_chart_time[key] < 1500:
                    return
            
            candle_data = self.get_intraday_data(sec_id, segment, name)
            if candle_data and len(candle_data) > 0:
                chart = self.generate_candlestick_chart(candle_data, f"{name} - 5 Min Chart")
                if chart:
                    await self.bot.send_photo(
                        chat_id=TELEGRAM_CHAT_ID, photo=chart,
                        caption=f"📈 *{name}* - 5 Min Chart\n🕐 {len(candle_data)} candles",
                        parse_mode='Markdown'
                    )
                    self.last_chart_time[key] = now
                    logger.info(f"Chart sent for {name}")
        except Exception as e:
            logger.error(f"Chart error for {name}: {e}")
